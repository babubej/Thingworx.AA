	...
	for (var x = 0; x < tableLength; x++) {
		...
		if (unitLevel !== "" && unitName !== "" && unit_id !== "" && parent_id !== "") {
			if (unitLevel === "DEALER" && unit_id !== "" && parent_id !== "" && parent_id === parent_id1) {
				...
				logger.debug("Dealer Parent Id : " + parent_id);
				arrayOfDealer.push({
					"unitLevel": unitLevel,
					"unitName": unitName,
					"unit_id": unit_id,
		...
		if (unitLevel !== "" && unitName !== "" && unit_id !== "" && parent_id !== "") {
			if (unitLevel === "CUSTOMER" && unit_id !== "" && parent_id !== "" && parent_id !== parent_id1) {
				...
				try {
					...
					logger.debug("527 CustomerNameOfOrganizationUnit_First : " + CustomerNameOfOrganizationUnit_First);
...
try {
	...
	if (obj.hasOwnProperty("orgUserDetails")) {
		...
		for (var xdj = 0; xdj < tableLength2; xdj++) {
			...
			if (userAction === "ADD" && name !== "" && name !== undefined && email !== "" && email !== undefined && unit_id !== "" && unit_id !== undefined) {
...
if (obj1.hasOwnProperty("fabNumber") && input.fabNumber !== undefined && input.fabNumber !== null && input.fabNumber !== "") {
	...
	for (var xtyu = 0; xtyu < tableLengthuyi; xtyu++) {
		...
		if (arrayOfRegion.length > 0) {
			logger.debug("929 regionUnitName ");
			if (unit_id1 === arrayOfRegion[0].unit_id) {
				logger.debug("931 regionUnitName : " + regionUnitName);
		...
		if (arrayOfCustomer.length > 0) {
			if (unit_id1 === arrayOfCustomer[0].unit_id) {
				logger.debug("CUSTOMER NAME : "+arrayOfCustomer[0].unitName);
	...
	logger.debug("Region : " + regionUnitName + ", Area : " + AreaUnitName + ", Dealer : " + DealerUnitName + ", Customer : " + CustomerUnitName);
	var input1 = {
