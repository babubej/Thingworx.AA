...
try {
						...
						try {
							...
							for (var c = 0; c < tableLengthS_First1; c++) {
								...
								me.SetUserHomeMashup({
									username: ServiceEnggUserName_First /* USERNAME */ ,
								...
								for (var e = 0; e < tableLengthS_First3; e++) {
									...
									if (serviceengg_email !== ServiceEnggUser_email_First1) {
										...
										for (var xtsty = 0; xtsty < tableLengthS_First1c; xtsty++) {
											...
											me.SetUserHomeMashup({
												username: ServiceEnggUserName_Firstt /* USERNAME */ ,
												mashupname: L2 /* MASHUPNAME */
											});
	...
	try {
		...
		if (obj.hasOwnProperty("orgUserDetails")) {
			...
			var dealer_idUser = serviceEngineerDetailsDataUser.dealer_id; // Get Dealer Unit Id from service engg block
			var customer_idUser = serviceEngineerDetailsDataUser.customer_id; // Get Customer Unit Id from service engg block
						...
						if (NameOfOrganizationUnit_UserAdd) {
							...
							for (var y = 0; y < tableLengthR_First2; y++) {
								...
								try {
									...
									if (tableLengthR_First1 === 0) {
										if (dealer_idUser === unit_id || customer_idUser === unit_id) {
											try {
												var userDescription = Users[email].GetDescription();
											} catch (ex) {
											...
											me.SetUserHomeMashup({
												username: UserName_First /* USERNAME */ ,
												mashupname: mashupName /* MASHUPNAME */
											});
										} else {
											try {
												var userDescription = Users[name].GetDescription();
												//Add user into Group 
												Groups[NameOfUnitGroups_First].AddMember({
													member: email /* STRING */ ,
													type: "User" /* STRING */
												...
												logger.error("ELGI User " + name + " not available in Thingworx");
											}
										}
									}
									/*                              else if(tableLengthR_First1 > 0) {
								for (var xunit = 0; xunit < tableLengthR_First1; xunit++) {
									...
									if (email === user_email_First) {
										Groups[NameOfUnitGroups_First].AddMember({
											member: UserName_First ,
											type: "User"
										});
										//break;
										// result: MASHUPNAME
										var mashupName = me.GetMashupFromUnitLevel({
											input: { "orgUnitDetails": orgUnitDetailsData } //JSON  ,
											unit_id: unit_id //STRING
										});
										me.SetUserHomeMashup({
                                    ...
                                    else if (email !== user_email_Firsts && (dealer_idUser === unit_id || customer_idUser === unit_id)) {
										
									} else {
										throw new Error('Only Dealer and Customer users can be Created in TWX. Please contact Administrator. ');
									}
